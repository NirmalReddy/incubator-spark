...
trait JavaDStreamLike[T, This <: JavaDStreamLike[T, This, R], R <: JavaRDDLike[T, R]]
  ...
  def transformWith[U, V](
      other: JavaDStream[U],
      transformFunc: JFunction2[R, JavaRDD[U], JavaRDD[V]]
    ): JavaDStream[V] = {
    implicit val cmu: ClassTag[U] =
      implicitly[ClassTag[AnyRef]].asInstanceOf[ClassTag[U]]
    implicit val cmv: ClassTag[V] =
      implicitly[ClassTag[AnyRef]].asInstanceOf[ClassTag[V]]
    def scalaTransform (inThis: RDD[T], inThat: RDD[U]): RDD[V] =
      transformFunc.call(wrapRDD(inThis), other.wrapRDD(inThat)).rdd
    dstream.transformWith[U, V](other.dstream,scalaTransform(_, _))
  }
